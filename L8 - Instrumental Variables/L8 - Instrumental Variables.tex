...
IV w/ controls
\begin{itemize}
\item \texttt{m5 = feols(workedm $\sim$ morekids + boy1st + boy2nd + black + hispan + othrace + agem1 + agefstm $|$
			samesex $\sim$ boy1st + boy2nd, data=babyData)}
	\item Second stage $\mid$ First stage
	\item Don't include endogenous $X$ before the pipe (only exogenous controls)
	\item Need to include \alert{excluded instruments} in the second equation!
...
One possibility is the \alert{Durbin-Wu-Hausman Test} which compares $\Delta = \beta_{OLS} - \beta_{2SLS}$ under the null that $X$ is \alert{exogenous} ($Z$ is assumed exogenous).
\begin{align*}
...
\begin{align*}
T = (\widehat{\beta}_{2SLS} - \widehat{\beta}_{OLS})^T\, (V(\widehat{\beta}_{2SLS}) - V(\widehat{\beta}_{OLS}))^{\dagger}\, (\widehat{\beta}_{2SLS} - \widehat{\beta}_{OLS}) \sim \chi^2_{K}
\end{align*}
