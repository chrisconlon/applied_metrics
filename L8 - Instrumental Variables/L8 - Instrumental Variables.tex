...
the treatment effect $\beta_i=\E\left(Y_{i}(1) - Y_{i}(0)\right)$ is heterogeneous.
\begin{itemize}
	\item $Y_{i}(x)$ denotes the potential outcome for $i$ given treatment status $X_i = x$.
\end{itemize}
...
\medskip
\item Let's also have potential outcomes for treatment status: $X_{i}(z)$ is the potential
...
\footnotesize{
\begin{array}{llcl}
\E\left[\beta_{i}\left(X_{i1}-X_{i0}\right)\right] & =\Pr\left(X_{i1}-X_{i0}=1\right)\E\left[\beta_{i}\left(X_{i1}-X_{i0}\right)|X_{i1}-X_{i0}=1\right]  & \text{(compliers)}\\
...
\item The second term (always/never takers) is necessarily equal to zero. We might also make a {\bf monotonicity assumption} that
	$\Pr\left(X_{i1}-X_{i0}=-1\right)=0$. 
\item Ruling out defiers, this could be interpreted to mean that 
...
\item Start with 
	\begin{align*}
	\E \left[ Y_i \mid Z_i = 1\right] - \E \left[ Y_i \mid Z_i = 0\right] = \E \left[ \beta_i \left(X_{i1} -X_{i0}\right) \right] 
	\end{align*}
	\item \alert{Monotonicity} means:
	\begin{align*}
		\E\left[\beta_{i}\left(X_{i1}-X_{i0}\right)\right]  = \Pr\left(X_{i1}-X_{i0}=1\right) \E\left[\beta_{i}\left(X_{i1}-X_{i0}\right)\mid X_{i1}-X_{i0}=1\right]
	\end{align*}

	\item Also note that $Pr\left(X_{i1}-X_{i0}=1\right) = E\left[ X_{i} \mid Z_i=1\right] - E\left[ X_{i} \mid Z_i=0\right]$

	\item Combining the above,
	\begin{align*}
	\frac{\E\left[ Y_i \mid Z_i = 1\right] - \E\left[ Y_i \mid Z_i = 0\right] }{\E\left[ X_{i} \mid Z_i=1\right] - \E\left[ X_{i} \mid Z_i=0\right]} 
	&= \E\left[\beta_{i}\left(X_{i1}-X_{i0}\right) \mid X_{i1}-X_{i0}=1\right]\\
	&= \E\left[\beta_{i} \mid X_{i1}-X_{i0}=1\right]
	\end{align*}
...
% \begin{frame}[fragile]{Example in \texttt{R}: Angrist \& Evans}
% \begin{itemize}
...
% \begin{lstlisting}
% # Run the Regression
% m6 = feols(workedm ~ 1 | morekids ~ boyBoy + girlGirl, data=babyData)

% # Back out the residuals
% babyData$uHat = babyData$workedm - fitted(m6)

% # Regress instruments on residual
% jReg = feols(uHat ~  boyBoy + girlGirl, data=babyData)

% # Back out the F-statistic on the instruments
% jlh = linearHypothesis(jReg,c("boyBoy","girlGirl"))
...
\begin{itemize}
	...
	$\symbf{Z'\, e} = \symbf{0} $, just like how OLS is the unique estimator that gives $\symbf{X'\, e} = \symbf{0} $
...
\begin{itemize}
	\item \alert{Moments} are expectations of functions of the data:
	\begin{align*}
		\E\left[ \symbf{m}\left(y_i,\symbf{x}_i;\symbf{\theta}_0\right) \right]=0,
	\end{align*}
	where $\symbf{\theta}_0$ is the true value of a vector of parameters of interest.
...
\begin{itemize}
	...
	\begin{align*}
		n^{-1} \sum_{i=1}^n \symbf{m}\left(y_i,\symbf{x}_i;\symbf{\theta}\right)
	\end{align*}
	will converge to the true analog, as long as $\symbf{\theta} = \symbf{\theta}_0$ (law of large numbers)

	\medskip
	\item Furthermore, if the problem is set up well (i.e., if the model is {\bf identified}),
	then
	\begin{align*}
		n^{-1} \sum_{i}^{n}\symbf{m}\left(y_{i},\symbf{x}_{i};\symbf{\theta}\right)\nrightarrow 0
	\end{align*}
for any value of $\symbf{\theta}\ne\symbf{\theta}_{0}$.
	\end{itemize}
\end{frame}

\begin{frame}{Generalized Method of Moments II}
\begin{align*}
n^{-1}\sum_{i}^{n}\symbf{m}\left(y_{i},\symbf{x}_{i};\symbf{\theta}_{0}\right) \rightarrow 0, \text{ and } 
\quad n^{-1}\sum_{i}^{n}\symbf{m}\left(y_{i},\symbf{x}_{i};\symbf{\theta}\right)\nrightarrow 0, \quad \forall\symbf{\theta}\ne\symbf{\theta}_{0}.
\end{align*}
\begin{itemize}
\item Thus, let's try to find a parameter vector $\hat{\symbf{\theta}}$ such that 
	\begin{align*}
	n^{-1}\sum_{i}^{n} \symbf{m}\left(y_{i},\symbf{x}_{i};\hat{\symbf{\theta}}\right) = 0.
	\end{align*}
\end{itemize}
\end{frame}
...
\begin{align*}
\E\left[\symbf{x}_{i}\varepsilon_{i}\right]=0\\
	ext{or}\\
\E\left[\symbf{x}_{i}\left(y_{i}-\symbf{x}_{i}'\symbf{\beta}\right)\right]=0
\end{align*}
\medskip
\item Similarly, IV estimation comes from the moments
\begin{align*}
\E\left[\symbf{z}_{i}\varepsilon_{i}\right]=0\\
	ext{or}\\
\E\left[\symbf{z}_{i}\left(y_{i}-\symbf{x}_{i}'\symbf{\beta}\right)\right]=0
\end{align*}
