...
\]
\begin{itemize}
	\item $R^2_{XZ}$ is the $R^2$ from the first stage regression of $X$ on $Z$
	\smallskip
	\item OLS variance under homoscedasticity: $Var(\hat\beta^{OLS})=\frac{1}{N}\frac{\sigma_{\varepsilon}^2}{\sigma^2_X}$
		\begin{itemize}
...
\begin{itemize}
\item Note that $L_t$, a demand shifter, can be used as an instrument for supply.
\item Also note that $I_t$, a supply shifter, can be used as an instrument for demand.
\item We can apply 2SLS to each equation, but it is somewhat more asymptotically
	efficient (i.e., smaller standard errors)
	to estimate the two jointly (see {\bf multi-equation GMM}).
\end{itemize}
\end{frame}

\begin{frame}{Example: Porter (1983)}
\begin{center}
\includegraphics[scale=.4]{Porter_t3}
\end{center}
\end{frame}
...
\begin{itemize}
	...
	\item Extra assumptions:
		\begin{itemize}
			...
			\item If $u$ and $X$ are correlated this is very complicated
		\end{itemize}
...
\item Intuitively, in a large sample, this $\hat{\boldsymbol{\theta}}$ should be close to the true parameter.
\end{itemize}
\end{frame}
