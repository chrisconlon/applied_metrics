...
## Problem 2 (Analytical Exercise)

Assume we have the following dynamic panel model (with the same indices from Question 1):
...
\begin{align*}
	...
	\Emeasure{T}{\epsilon_{i,t} \vert y_{i,t-1},...,y_{i,0},\alpha_{i}} &= 0 \mbox{ (Sequential Exogeneity)} 
\end{align*}
...
## Problem 3 (Analytical Exercise)

You will still be working with the same model from the previous question. We now want to investigate the properties of the fixed effects ("within") estimator. 

\begin{enumerate} 
	\item Derive the following formula for the (population) fixed effects ("within") estimator $\beta_{FE}$ in terms of $\bar{y}_{i}$, $\bar{y}_{i,-1}$, $y_{i,t-1}$, $y_{i,t}$ where:
	\begin{align*}
	\beta_{FE} &= \Emeasure{T}{\big(y_{i,t-1} - \bar{y}_{i,-1}\big)^{2}}^{-1} \Emeasure{T}{\big(y_{i,t-1} - \hat{y} _{i,-1}\big)\big(y_{i,t} - \bar{y}_{i}\big)} \\ 
		\bar{y}_{i} &= \frac{1}{T} \sum_{t=1}^{T} y_{i,t} \\
		\bar{y}_{i,-1} &= \frac{1}{T} \sum_{t=1}^{T} y_{i,t-1} \\		
	\end{align*}
	
	(Note: $\Emeasure{T}{\cdot}$  is an expectation with respect to some probability measure (think distribution) fixing our sample size $T$) 
	\item Show that the potential bias of the FE estimator is:
	\begin{align*}
		\hat{\beta}_{FE} - \beta = \Emeasure{T}{\big(y_{i,t-1} - \bar{y}_{i,-1}\big)^{2}}^{-1}\Emeasure{T}{\big(y_{i,t-1} - \bar{y}_{i,-1}\big)\big(\epsilon_{i,t} - \bar{\epsilon}_{i}\big)}
	\end{align*}

	\item Show that the numerator has the following form:
	\begin{align*}
		\Emeasure{T}{\big(y_{i,t-1} - \bar{y}_{i,-1}\big)\big(\epsilon_{i,t} - \bar{\epsilon}_{i}\big)} = -\Emeasure{T}{\bar{y}_{i,-1}\bar{\epsilon}_{i}}
	\end{align*}
	\newline
	(Hint: Factor out the expression into four terms, work with each term individually)
	\item Why do we expect the numerator not to be equal to 0? 
	\newline (No need for rigourous calculations)
	\item Using backward substitution:
	\begin{align*}
		y_{i,t} &= \gamma y_{i,t-1} + \alpha_{i} + \epsilon_{i,t} \\
		y_{i,t} &= \gamma \big(\gamma y_{i,t-2} + \alpha_{i} + \epsilon_{i,t-1}\big) + \epsilon_{i,t} \\
		&\vdots
	\end{align*}		
	Derive the following form for the numerator:
	\begin{align*}
		-\Emeasure{T}{\bar{y}_{i,-1}\bar{\epsilon}_{i}} &= -\frac{\Emeasure{T}{\epsilon_{i,t}^{2}}}{T^{2}}\frac{\big(T - T\gamma - 1 + \gamma^{T}\big)}{\big(1-\gamma^{2}\big)}
	\end{align*}
	(Hint: Remember the following formula: if $\vert \gamma \vert < 1$ then $\sum_{t=0}^{T} \gamma^{t}b = b\big(\frac{1-\gamma^{T+1}}{1-\gamma}\big)$)
	\item Once again, using backward substitution derive the following form for the denominator:
	\begin{align*}
		\Emeasure{T}{\big(y_{i,t-1} - \bar{y}_{i,-1}\big)^{2}} = \frac{\Emeasure{T}{\epsilon_{i,t}^{2}}}{1-\gamma^{2}}\Big(1 - \frac{1}{T} - \frac{2\gamma}{(1-\gamma)^{2}}\frac{(T - T\gamma - 1 + \gamma T)}{T^{2}}\Big)
	\end{align*}
	\item What happens to the bias for small $T$? For $T \to \infty$?
	\newline
	(Note: You need to only reduce the algebra to something you can make a statement about the bias.)
\end{enumerate} 

## Problem 4 (Coding Exercise)
