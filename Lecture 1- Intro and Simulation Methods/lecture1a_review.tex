...
\item Obtain $\widehat{\varepsilon}_i$.
\item Construct $\E[ \varepsilon_i^2 \mid X, Z]$ via (nonlinear) regression: $\exp[ \gamma_0 + \gamma_1 x_{i} + \gamma_2 z_{i}]$.
\item Update $\widehat{\Omega}$ with $\E[ \varepsilon_i^2 \mid X, Z]$
\item Repeat until parameter estimates don't change
\end{enumerate}
\end{frame}
