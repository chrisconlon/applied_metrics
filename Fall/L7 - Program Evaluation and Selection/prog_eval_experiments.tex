...
\title[L5a - Experiments]{ Econometrics I}
\subtitle{Lecture 5a: Potential Outcomes, Randomized Experiments, and Design}
...
\item Heckman and Vytlacil (2005/2007)
...
\begin{frame}{Some Challenges: SUTVA}
...
\begin{itemize}
\item Peer effects: Whether you respond to a job training program depends on whether your spouse is also treated.
...
\begin{frame}{Some Challenges: The Fundamental Problem}
...
\item Most program evaluation approaches seek to identify one effect or the other effect. These are therefore described as \alert{reduced form} or \alert{quasi-experimental}.
\item The \alert{structural} approach attempts to recover the entire joint $f(\beta_i,\varepsilon_i)$ distribution; it generally requires more assumptions, but then we can calculate whatever we need.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item People often choose $D_i$ with $Y_i(0)$ or $\beta_i$ in mind.
\item The problem: $D_i \perp u_i$ and/or $D_i \perp \beta_i$ are likely violated.
\item Selection on baseline outcomes ($u_i$) can go either way:
\begin{itemize}
\item \alert{Positive}: who joins the loyalty program? Your best customers, who would have bought a lot anyway.
\item \alert{Negative}: which accounts get a retention offer? The ones about to churn.
\end{itemize}
\item Selection on gains ($\beta_i$): which sellers buy paid promotion? Those who expect the biggest lift,
so the comparison recovers (at best) the ATT, not the ATE.
\end{itemize}
\end{frame}
...
\end{align*}
\begin{itemize}
  \item If treatment is assigned randomly, then $Y_{i}(0)$ and $D_{i}$ should be independent. Consequently,
  \begin{align*}
...
\begin{itemize}
  \item Simplifying the situation by assuming $\beta_1=Y_{i}(1)-Y_{i}(0)$ for all
...
where $\beta_{0}=\E\left[Y_{i}(0)\right]$ and $\varepsilon_{i}=Y_{i}(0)-\beta_{0}$.
  ...
  then the error term $\varepsilon_{i}$ is correlated with the regressor $D_i$,
  violating the strict exogeneity assumption, and leading to biased estimates of $\beta_1$.
...
\medskip
  ...
  between whether the subject \emph{believes} they are being treated or not, as well as whether they are \emph{actually}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    whether to give the subjects a pill or not? Perhaps we won't get the placebo effect twice. 
...
\begin{itemize}
  ...
  even if (pre-treatment) ${\bf X}$  is omitted from the regression model, there is {\bf no problem of
  omitted variables bias} because $D$ and ${\bf X}$ are uncorrelated.

  \medskip
  \item However, controlling for covariates that predict $Y$ may improve precision of the estimates
  by reducing residual variance. In small samples it also adjusts for chance imbalance in ${\bf X}$
...
\begin{itemize}
	...
	\medskip
	\item So every other assignment the design could have produced is an equally likely
	...
	(up to simulation error, if we draw shuffles rather than list every assignment).
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Randomization Inference in a few lines}
\begin{lstlisting}
...
\begin{itemize}
	...
	clusters is enough?'' anxiety (few clusters just means coarse p-values).

	\medskip
	\item \alert{You will see it again.} The synthetic-control placebo test
...
\begin{frame}{Experiments in the Social Sciences}
\begin{itemize}
  ...
  \begin{itemize}
    ...
    when there are spillovers across markets/countries. E.g., what is the global
    impact of the EU's decision to implement carbon pricing?
    \item Mergers and antitrust, other situations where regulatory questions are highly
     context-specific.
  \end{itemize}
...
\begin{frame}{Example: Banerjee et al.\ (2007), cont.}
\begin{center}
...
Let $\hat{p}_1$ and $\hat{p}_0$ be the sample proportions of successful sales in treatment and control.

Under random assignment and large $n_1, n_0$:
\begin{align*}
\hat{p}_j &\overset{a}{\sim} \mathcal{N}\!\left(p_j,\; \frac{p_j(1-p_j)}{n_j}\right), \quad j\in\{0,1\}
\end{align*}
For large $n$, the difference-in-means is approximately normal:
\begin{align*}
\hat{p}_1 - \hat{p}_0 
\overset{a}{\sim} \mathcal{N}\!\left(p_1 - p_0,\;
\frac{p_1(1-p_1)}{n_1} + \frac{p_0(1-p_0)}{n_0}\right).
\end{align*}

The Wald statistic divides by the (unpooled) estimated standard error; under $H_0: p_1 = p_0$,
\begin{align*}
...
\begin{frame}{Large-sample approx.: two-sample difference-in-proportions}
  ...
  \begin{align*}
    \hat{p}_1 - \hat{p}_0 \overset{a}{\sim} \mathcal{N}\!\left(p_1-p_0,\; \frac{p_1(1-p_1)}{n} + \frac{p_0(1-p_0)}{n}\right).
  \end{align*}
...
\begin{frame}{Worked example (binary conversion)}
  ...
  \begin{align*}
    n \approx 80{,}679 \quad\text{(per arm)}.
  \end{align*}
  \vspace{4pt}
...
\begin{frame}{Minimum Detectable Effect (MDE)}
  ...
  \end{align*}
  For planning you can use the simple plug-in $p_1\approx p_0$ (slightly anti-conservative for positive lifts when $p_0<0.5$), giving
  \begin{align*}
...
\begin{frame}{Practical considerations}
  \begin{itemize}
    \item \textbf{Equal vs unequal allocation:} If treatment fraction $\pi \ne 0.5$, replace $2/n$ ($n$ per arm) by $1/(N\pi)+1/(N(1-\pi))$, where $N$ is the total sample size, or solve for each arm separately.
    \item \textbf{Clustering:} If randomization is by cluster (e.g., by geography), inflate $n$ by the design effect: $DE = 1 + (m-1)\rho$, where $m$ is cluster size, $\rho$ ICC.
    \item \textbf{Multiple testing / sequential peeking:} Correct $\alpha$ (Bonferroni, group sequential, alpha-spending).
    \item \textbf{Non-compliance / attrition:} Adjust expected treated proportion and effective sample size (with compliance rate $c$, the ITT shrinks by $c$, so $n$ grows by $1/c^2$).
    \item \textbf{Unequal variances:} If variance differs by arm, use $p_1(1-p_1)$ and $p_0(1-p_0)$ explicitly.
    \item \textbf{Pre-launch A/A test to estimate $p_0$ and $\sigma$:} Required $n$ is very sensitive to the baseline rate and SD.
  \end{itemize}
\end{frame}
...
\begin{frame}{Summary}
  \begin{itemize}
    ...
    \item R functions \texttt{power.prop.test()} and \texttt{power.t.test()} provide convenient built-in calculations.
