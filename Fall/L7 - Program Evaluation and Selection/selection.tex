...
\title[L12c - Selection]{ Econometrics I}
\subtitle{Lecture 12c: Selection Models}
...
\begin{itemize}
    \item OLS on a selected sample is fine if selection depends only on $x_i$; not if it depends on unobservables.
    ...
    \begin{align*}
    y_i \text{ (the wage) observed only if } s_i = 1
    \end{align*}
...
So OLS of $y_i$ on $x_i$ in the selected sample omits the regressor $\lambda(z_i'\gamma)$.
\end{frame}
%------------------------------------------------
...
\begin{enumerate}
    ...
    \end{align*}
    \item \textbf{Step 2: Outcome Equation (Corrected OLS)}, using only observations with $s_i=1$
    \begin{align*}
...
\vspace{0.2cm}
Note: standard errors must be corrected for the generated regressor $\hat{\lambda}_i$ and the built-in heteroskedasticity.
\end{frame}
...
\begin{frame}{OLS vs. Heckman Selection Model}
\begin{center}
\begin{tabular}{@{\extracolsep{5pt}}lccc} 
\toprule
 & OLS (sample B) & Heckman, no excl.\ (A) & Heckman, excl.\ (B) \\ 
\midrule
...
\bottomrule 
\textit{Note:}  & \multicolumn{3}{r}{A: selection on educ, exper; B: adds kids. $^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01} \\
\end{tabular} 
\end{center}
\end{frame}
...
\end{align*}
\item Now we allow for migration cost of $C$, expressed in time-equivalent units: $\pi = \frac{C}{w_0}$.
...
\frametitle{Example: Borjas (1987): How does selection work?}
\begin{itemize}
\item The analogous expression for immigrants' host-country wages:
\begin{align*}
...
Let $Q_{0}=\E\left(u_{0} | I=1\right), Q_{1}=\E\left(u_{1} | I=1\right)$ (expected \alert{skill} of immigrants; $I=1$: migrate).
\begin{itemize}
...
\item $\rho>\frac{\sigma_{0}}{\sigma_{1}}$ correlation between valued skills in both countries is high (similar skills valued in both countries).
\end{itemize}
...
\begin{itemize}
\item Immigrants are negatively selected and below average $(Q_0,Q_1) < 0$ and $\frac{\sigma_{0}}{\sigma_{1}}>1 \text { and } \rho>\frac{\sigma_{1}}{\sigma_{0}}$
\begin{itemize}
\item $\frac{\sigma_{0}}{\sigma_{1}}>1$ returns to ``skill'' are lower in host country.
\item $\rho>\frac{\sigma_{1}}{\sigma_{0}}$ correlation between valued skills in both countries is high (similar skills valued in both countries).
\end{itemize}
...
\begin{itemize}
\item Immigrants are below average at home and above average in host $(Q_0<0 ,Q_1>0)$ iff:
\begin{itemize}
