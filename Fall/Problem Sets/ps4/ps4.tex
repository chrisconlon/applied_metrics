...
\begin{enumerate}[label=\alph*)]
    ...
    \[
    \begin{array}{cl}
    h_{00} =& \mathbb{E}[X_i \mid D_{0i} = 0, D_{1i} = 0] \\
    h_{01} =& \mathbb{E}[X_i \mid D_{0i} = 0, D_{1i} = 1] \\
    h_{10} =& \mathbb{E}[X_i \mid D_{0i} = 1, D_{1i} = 0] \\
    h_{11} =& \mathbb{E}[X_i \mid D_{0i} = 1, D_{1i} = 1] \\
    \end{array}
    \]
     weighted by the probabilities $\pi_{01} := Pr(D_{0i} = 0, D_{1i} = 1)$, etc..

    \item Suppose the monotonicity condition holds, i.e. $D_{1i} \geq D_{0i}$ for all individuals, and that ($X_i$, $D_i$) is independent of $Z_i$. Show that 
    \begin{align*}
        \mathbb{E}[D_i(1 - Z_i)X_i] = \mathbb{E}[X_i \mid D_{0i} = 1, Z_i = 0] \,Pr(D_{0i} = 1 \mid Z_i = 0) Pr(Z_i = 0).
    \end{align*}

    \item Show that under these conditions 
    \begin{align*}
        \mathbb{E}[D_i(1 - Z_i)X_i] = \mathbb{E}[X_i \mid D_{0i} = 1, D_{1i} =1]\, Pr(D_{0i} = 1, D_{1i} = 1)\, Pr(Z_i = 0).
    \end{align*}

    \item Show that under the same conditions 
    \begin{align*}
        \mathbb{E}[(1 - D_i)Z_i X_i] = \mathbb{E}[X_i \mid D_{0i} = 0, D_{1i}= 0]\, Pr(D_{0i} = 0, D_{1i} = 0)\, Pr(Z_i = 1).
    \end{align*}

    \item Use your results from parts (b), (d), and (e) to show that for the function 
    \begin{align*}
        w_i := 1 - \frac{D_i(1 - Z_i)}{P (Z_i=0)} -  \frac{(1-D_i)Z_i }{Pr (Z_i=1)}
    \end{align*} we have 
    \begin{align*}
        \mathbb{E}[w_i X_i] = \mathbb{E}[X_i \mid D_{0i} = 0, D_{1i} = 1]\, Pr(D_{0i} = 0, D_{1i} = 1)
    \end{align*} How is that result useful to describe the complier group with respect to the instrumental variable $Z_i$ in terms of other observable characteristics?
