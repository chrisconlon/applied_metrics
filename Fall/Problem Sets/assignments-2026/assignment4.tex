...
\begin{align*}
s_{i} &= u_{i} + v_{i}, \qquad D_{i} = \mathbb{1}\{s_{i} > c_{j(i)}\},
...
\item Before simulating: $D_{i}$ depends on $u_{i}$, and so does $y_{i}$. Use the operator
rules to sign the bias of OLS of $y$ on $D$. Give the economic story in one sentence.
\item Simulate the data. Regress $y$ on $D$. Compare to $1$.
