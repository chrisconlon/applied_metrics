...
% \usepackage{slashbox}
\title[L12b - Logit and Probit]{ Econometrics I}
\subtitle{Lecture 12b: Binary Choice --- Logit and Probit}
...
We can write the likelihood of $N$ Bernoulli trials (proportional to a Binomial): 
...
\item Wants to maximize profits (i.e., loan to people who don't end up defaulting!)
\end{itemize}
\end{itemize}
\end{itemize}
\end{frame}
...
\item What does the line $\beta_0 + \beta_1 X$ represent when $Y$ is binary?
...
Marginal Effects: 
\begin{itemize}
\item Increasing $P/I$ from $0.3 \rightarrow 0.4$ increases probability of denial by $5.59$ percentage points. (True at all levels of $P/I$).
\item At all $P/I$ levels black applicants are $17.7$ percentage points more likely to be denied.
...
\item For the LPM, $F(\cdot)$ is the \alert{identity function} (which doesn't satisfy my $[0,1]$ property).
...
\item $\beta_1$ is the change in the $z$-value for a change in $X$.
\end{itemize}
\end{itemize}
\end{frame}
...
\item $\beta_1$ is the change in the \alert{log odds} per unit change in $X$; $e^{\beta_1}$ is the \alert{odds ratio}.
\item $\log\frac{p}{1-p} \in (-\infty,\infty)$ which fixes the $[0,1]$ problem in the other direction.
...
\begin{itemize}
    ...
    \item Rule of thumb: $\beta_{logit} \approx  1.81 \cdot \beta_{probit}$ (why? see latent variables below)
    \item It is better to compare marginal effects $\frac{\partial \Pr(Y_i=1 \mid X)}{\partial x_k}$
...
\E_{X_i}[ f (Z_i) \beta_k]
$$
\item The right thing to do is probably to plot the response surface (either the probability or the change in probability) over all $X$.
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Marginal effects of binary regressors}
\small
For a dummy like \texttt{black}, there is no ``small change'': the effect is a \alert{discrete change}
\begin{align*}
\Delta_i = F\left(X_i\beta \mid \text{black}_i=1\right) - F\left(X_i\beta \mid \text{black}_i=0\right),
\qquad \text{AME} = \frac{1}{N}\sum_{i=1}^N \Delta_i
\end{align*}
\begin{itemize}
\item The derivative $f(Z_i)\beta_k$ only approximates this jump, and poorly when $\beta_k$ is large.
\end{itemize}
\begin{center}
\begin{tabular}{lccc}
\toprule
 HMDA: \texttt{black} on Pr(deny) & LPM & Probit & Logit \\
\midrule
Discrete-change AME & 0.081 & 0.062 & 0.057 \\
Derivative AME, $\frac{1}{N}\sum_i f(Z_i)\beta_k$ & 0.081 & 0.054 & 0.050 \\
\bottomrule
\end{tabular}
\end{center}
\begin{itemize}
\item \texttt{marginaleffects::avg\_slopes()} detects a 0/1 regressor and reports the discrete change; hand-rolled $f(Z_i)\beta_k$ code does not.
\end{itemize}
\end{frame}
...
\item Sometimes you will see this description in the literature. Next: it delivers exactly the models above.
\end{itemize}
\end{frame}


\begin{frame}
\frametitle{Latent Variables: Where $F$ Comes From}
\small
Write the latent variable as a linear index plus an error with a symmetric CDF $F$:
\begin{align*}
Y_i^{*} = X_i\beta + \varepsilon_i, \qquad Y_i = 1\left[Y_i^{*} > 0\right]
\end{align*}
Then
\begin{align*}
\Pr(Y_i = 1 \mid X_i) = \Pr(\varepsilon_i > -X_i\beta) = 1 - F(-X_i\beta) = F(X_i\beta)
\end{align*}
\begin{itemize}
\item $\varepsilon_i \sim \mathcal{N}(0,1)$ gives the \alert{probit}; $\varepsilon_i$ standard logistic gives the \alert{logit}.
\item \alert{Only the scale of $\beta$ relative to $\varepsilon$ is identified}: $Y^{*}$ and $cY^{*} = X(c\beta) + c\varepsilon$
produce the same $Y$ for any $c>0$. So we normalize: $\Var(\varepsilon)=1$ (probit), $\pi^2/3$ (logit).
The threshold $0$ is also a normalization, absorbed by the intercept.
\item The logit error has SD $\pi/\sqrt{3} \approx 1.81$, so $\beta_{logit} \approx 1.81\,\beta_{probit}$.
Fatter logistic tails make the ratio vary (HMDA: 1.79--1.95; Amemiya suggested 1.6).
\end{itemize}
\end{frame}
...
\item Logit: $F(z) = \frac{1}{1+e^{-z}}$ and its derivative (density) $f(z) = \frac{e^{-z}}{(1+e^{-z})^2}$. A convenient property is that $\frac{f(z)}{F(z)} = 1 - F(z)$ and $\frac{f(z)}{1-F(z)} = F(z)$ (the \alert{reverse hazard} and \alert{hazard rate}).
\end{itemize}
\end{frame}
...
\end{align*}
This uses the logit identities $f/F = 1-F$ and $f/(1-F) = F$.
\end{frame}
...
\begin{align*}
\nabla_{\beta} \cdot \ell(\symbf{y}; \beta)= \sum_{i=1}^N \underbrace{\frac{ q_i \phi(q_i Z_i)}{\Phi(q_i Z_i)}}_{\lambda_i} X_i
\end{align*}
\end{frame}
...
\begin{align*}
\frac{\partial^2 \ell }{\partial \beta \partial \beta'} = - \sum_{i=1}^N   y_i \frac{ f(Z_i)  f(Z_i) - f'(Z_i) F(Z_i) }{ F(Z_i)^2}  X_i X_i' \\
...
\begin{align*}
\frac{\partial^2 \ell }{\partial \beta \partial \beta'}  &= - \sum_{i=1}^N f(Z_i) X_i X_i' \\
...
\nabla_{\beta} \cdot \ell(\symbf{y}; \beta)= \sum_{i=1}^N \underbrace{\frac{ q_i \phi(q_i Z_i)}{\Phi(q_i Z_i)}}_{\lambda_i} X_i
\end{align*}
Take another derivative and recall $\phi'(z) = - z \phi(z)$ (and $q_i^2=1$)
\begin{align*}
\nabla_{\beta}^2 \cdot \ell(\symbf{y}; \beta)&= \sum_{i=1}^N \frac{q_i^2 \left[\phi'(q_i Z_i) \Phi(q_i Z_i) - \phi(q_i Z_i)^2\right]}{\Phi(q_i Z_i)^2}  X_i X_i' \\
&= - \sum_{i=1}^N \lambda_i( Z_i + \lambda_i) \cdot X_i X_i'
\end{align*}
...
\item $\rho = 1- \frac{\ell(\hat{\beta}^{MLE})}{\ell_0}$ where $\ell_0$ is the log likelihood of a model with just a constant (unconditional probability of success).
\begin{itemize}
...
\begin{itemize} 
\item Machine learning likes to think about this problem more like \alert{classification} than regression.
...
\item Predict $\hat{y}_i =1$ if $\Pr(y_i = 1 | X_i =x) \geq 0.5$, i.e., $F(X_i \hat{\beta}) \geq 0.5$.
\item Imagine four cells Prediction: $\{Success, Failure\}$, Outcome $\{Success, Failure\}$
\item Can construct this using the R package \texttt{caret} and command \texttt{confusionMatrix()}.
\end{itemize}
\end{frame}
...
\begin{frame}{ROC Curve / AUC}
\begin{center}
\includegraphics[width=2.5in]{resources/roc_curve.png}\\
\end{center}
\begin{itemize}
\item At each classification threshold calculate both \alert{True Positive Rate} and \alert{False Positive Rate}.
\item AUC is the area under the curve
\end{itemize}
\end{frame}
