...
\begin{itemize}
	\item Basic Idea of Instrumental Variable (IV):
		\begin{itemize}
			\item What if we have a variable that is correlated with $X$ \emph{but not with $\varepsilon$} (it affects $Y$ only through $X$)
...
\begin{frame}{Instrumental Variables with Pictures}
...
\begin{itemize}{\small
	...
	\item We can estimate ``causal effect'' of $Z$ on $Y$ and this MUST be the product of the causal effect of $Z$ on $X$ and the causal effect of $X$ on $Y$
	\begin{itemize}
		\item Mathematically we need to split the effect of $Z$ on $Y$ into effect of $Z$ on $X$ and $X$ on $Y$
...
\begin{itemize}
\item We call a variable $Z$ a valid {\bf instrumental variable} if the following two conditions hold:
\end{itemize}
\begin{enumerate}
...
\begin{frame}{Key Assumption \#2 of Instrumental Variables}
\begin{itemize}
...
\begin{frame}{Best Defense of the IV Exogeneity Assumption: Randomized Experiment}
\begin{itemize}
...
\end{itemize}
\medskip
\item {\bf Exogeneity:} $Cov(\varepsilon,Z)= 0$? Untestable -- so need to tell a story
\begin{itemize}
...
\item When running an experiment, the exogeneity assumption is justified if the
	randomization was implemented in a way that was not correlated with anything else that could
...
\begin{frame}{The Relationship Between $Y$ and $Z$}
...
\medskip
\item One possible IV: whether your roommate has an N64 (or PlayStation/whatever video game console kids use today)
\medskip
...
\begin{enumerate}
\item People do \emph{not} select into $Z$ in a way that is correlated with $Y$ (as-good-as-random)
\begin{itemize}
...
\end{itemize}
\medskip
\item \textbf{Interpretation of $\hat{\pi}_1$:} Having a roommate with an N64 \textbf{causes} a $\hat{\pi}_1$ change in GPA (we expect $\hat{\pi}_1<0$)
\begin{itemize}
\item Causal effect because validity of instrument, $Cov(Z,\varepsilon)=0$, implies OLS is consistent for the above equation.
...
\item Regress $Y$ on the fitted values from step 1 to capture the effect of $X$ on $Y$
\end{enumerate}
\smallskip
...
\begin{itemize}
\item Why does 2SLS estimate $\beta_1=\frac{Cov(Y,Z)}{Cov(X,Z)}$?
\medskip
\item True model:
\begin{align*}
Y_i=\beta_0 + \beta_1 X_i + \varepsilon_i
...
Cov(Y_i, Z_i)& = 0 + \beta_1Cov( X_i, Z_i) +0 \;\; \textit{since $Cov(\varepsilon_i, Z_i)=0$ by assumption} \\
...
\item $Y_i$: is GPA
\item $X_i$: is study time (hours per day)
\item $Z_i$: is indicator for roommate having N64
\end{itemize}
\medskip
...
\begin{itemize}
\item Note: Standard errors will be wrong! (Second-stage residuals use $\hat{X}$; the correct residuals use $X$.)
\end{itemize}
\end{itemize}
	\begin{itemize}{\small
...
\begin{itemize}
\item $\E\left[{\bf z}_{i}\varepsilon_{i}\right]={\bf 0}$
\item $\E\left[{\bf z}_{i}{\bf x}_{i}'\right]$ has \emph{full column rank} $K$, where $K$ is
...
\end{align*}
\smallskip
\item With homoskedasticity, the estimated asymptotic variance of the estimator is
\begin{align*}
\widehat{Var}\left(\boldsymbol{b}_{2SLS}\right)=s^{2}\left[\boldsymbol{X}'\boldsymbol{Z}\left(\boldsymbol{Z}'\boldsymbol{Z}\right)^{-1}\boldsymbol{Z}'\boldsymbol{X}\right]^{-1}, \quad s^2 = \frac{1}{n-K}\sum_i \left(y_i - {\bf x}_i'{\bf b}_{2SLS}\right)^2
\end{align*}
\smallskip
...
m5 = feols(worked ~ boy1st + boy2nd + black + hisp + othrace + age + agefst | morekids ~ samesex,
     data=babydata)
\end{lstlisting}
\end{column}
\end{columns}
...
\begin{frame}{Comparing 2SLS Variance to OLS Variance}{under Homoskedasticity}
...
\end{align*}
	\smallskip
	OLS variance under homoskedasticity: $Var(\hat\beta^{OLS})=\frac{1}{n}\frac{\sigma_{\varepsilon}^2}{\sigma^2_X}$
		\begin{itemize}
			\item Since $R^2_{XZ}\leq 1$, IV variance is at least as large
...
&=\beta_1 + \E\left[ \frac{\sum\limits_{i=1}^N(z_i-\bar{z})\varepsilon_i}{\sum\limits_{i=1}^N(x_i-\bar{x})(z_i-\bar{z})} \right] \overset{?}{=}\beta_1 + 0
\end{align*}
{\small The last step fails: the denominator contains $x$, which is correlated with $\varepsilon$. 2SLS is \alert{consistent but biased} toward OLS; just-identified IV has no finite mean.}
\end{frame}
...
\begin{itemize}
	...
	\item What conditions would $Z$ have to fulfill?
		\begin{enumerate}
			\item Instrument Relevance: $\alpha_1\neq 0$
...
\begin{frame}{More Simultaneity Examples}
\begin{itemize}
	...
	\item Extra assumptions:
		\begin{itemize}
			\item {\bf Classical Errors-in-Variables:}
				\begin{align*}
				X_i^* = X_i + u_i,\quad \E[u_i]=0,\ \Cov(u_i,X_i)=\Cov(u_i,\varepsilon_i)=0
				\end{align*}
		\end{itemize}
...
Under classical measurement error:
	\begin{align*}
		Y_i = \beta_0 + \beta_1 X_i^* + \underbrace{\varepsilon_i - \beta_1u_i}_{V_i}
	\end{align*}
\begin{itemize}
...
\begin{frame}{Measurement Error in $X$}{Classical Measurement Error Intuition}	
	\begin{itemize}
		...
		\begin{itemize}
			\item Noise in measured $X$ makes it more likely to see high $X$ and low $X$ with the same $Y$ because of randomness
		...
		\item Notice the following rearranging of the attenuation term:
			...
			\begin{itemize}
				...
				\item What matters is the \emph{relative} variance of $X$ to $u$
...
Y_{i}+(Y_{i}^{*}-Y_{i}) & = & \beta_{0}+\beta_{1}X_{i}+\varepsilon_{i}+(Y_{i}^{*}-Y_{i})\\
