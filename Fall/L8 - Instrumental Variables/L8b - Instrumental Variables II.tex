...
\item First, let's talk about the relevance assumption.
	\item It's easy to check whether the sample analog of $Cov(X,Z)$ is nonzero, or the analogous
	matrix condition.
...
\begin{itemize}
\item There is a \textbf{big} problem with IV estimation: 2SLS performs \textbf{terribly} in small samples
\medskip
...
Suppose that $Z$ is \emph{a bad instrument} in that $Z$ and $\varepsilon$ are correlated

\begin{itemize}
	\item Similar to OVB
		\begin{align*}
			\hat\beta_1^{IV}  \to \beta_1 + \frac{Corr(Z,\varepsilon)}{Corr(Z,X)}\times\frac{\sigma_\varepsilon}{\sigma_X}
		\end{align*}
	\item Compare to OLS case (biased because of OVB):
		\begin{align*}
			\hat\beta_1^{OLS} \to \beta_1 + Corr(X,\varepsilon)\times\frac{\sigma_\varepsilon}{\sigma_X}
		\end{align*}
...
\medskip
	\item Second issue: even if the normal approximation works the variance explodes!
		\begin{itemize}
			\item Intuition from homoskedastic case: $Var(\hat\beta^{IV}) = \sigma^2/(N\sigma^2_XR^2_{XZ})$
...
\begin{itemize}
		\item Easiest: get stronger instruments
\medskip
		\item Harder: adjust how to do inference
			\begin{itemize}
				...
				\item We return to this below (Anderson--Rubin)
			\end{itemize}
\medskip
		\item The good news: We can test $Cov(X,Z)$ by regressing $x_i=\pi_0 + \pi_1 Z_i + \eta_i$
			...
			\end{itemize}
\medskip
\item \textbf{Rule of Thumb:} Only use instrument if the first-stage $F$-statistic for $\pi_1=0$ is $\geq 10$ (with one instrument, $|t|\geq 3.16$; well above 1.96!)
...
\end{align*}
	...
	\begin{itemize}
		...
		\item If $\hat\pi$ is the wrong \emph{sign} or close to \emph{zero} then it is very likely that $\hat\beta^{IV}$ will behave poorly
		\item {\bf NB:} As $N\to\infty$, $\hat\pi\to\pi$, but what matters is the \emph{concentration parameter} ($N\pi^2$ relative to noise): a small $\pi$ needs a very large $N$.
	\end{itemize}
\end{frame}

\begin{frame}{The Sampling Distribution of $\hat\beta$ ($N=100$)}
\begin{align*}
	Y = 1 + 2 X + \varepsilon
\end{align*}
...
\centering \caption{\small Distribution of $\hat\beta$ for OLS, strong IV, weak IV with $N=100$}
	\includegraphics[width=.3\textwidth]{ols_smallN.pdf}\includegraphics[width=.3\textwidth]{strongIV_smallN.pdf}\includegraphics[width=.3\textwidth]{weakIV_smallN.pdf}
\end{figure}
...
\item Weak IV has heavy tails (just-identified IV has no finite mean): $\mu=2.05$ but $\boldsymbol{\sigma^2 = 1653.11}$
}
\end{itemize}
...
\begin{frame}{The Sampling Distribution of $\hat\beta$ ($N=10{,}000$)}
\begin{align*}
	Y = 1 + 2 X + \varepsilon
\end{align*}
...
\centering \caption{\small Distribution of $\hat\beta$ for OLS, weak IV, strong IV with $N=10000$}
	\includegraphics[width=.3\textwidth]{ols_largeN.pdf}\includegraphics[width=.3\textwidth]{weakIV_largeN.pdf}\includegraphics[width=.3\textwidth]{strongIV_largeN.pdf}
\end{figure}
	...
	\begin{itemize}
		...
		\item Remember that for testing we want to reject the null hypothesis incorrectly 5\% of the time
			\begin{itemize}
				...
				\item Example rejection probabilities for the test of $H_0\colon \beta=2$
				\begin{table}[H]
				\centering
				\begin{tabular}{rr}
				...
				\end{tabular}
				\end{table}
				\item Goal should be 5\% but for weak IV with $N$ small, almost never reject; still only 3\% even for $N$ large---this is a bad test! (With strong endogeneity it over-rejects instead; see below.)
...
\smallskip
	...
	\end{enumerate}
	\item These have fallen out of favor because they don't make sense if we have \alert{heterogeneous treatment effects} (i.e., $\beta_i$).
...
\begin{align*}
	\mathbf{Z}'\mathbf{e}_{IV}&=\mathbf{0}, \text{ with }
...
\begin{frame}{Tests of Exogeneity? Hausman}
...
\begin{frame}{Tests of Exogeneity? Sargan $J$}
The other possibility is the \alert{Sargan J-Test} which exploits \textbf{overidentification} (more instruments than endogenous regressors)
\begin{enumerate}
	\item Pick instrument(s) $z_{i}^* \in \mathbb{R}^{L-K}$ to drop.
	\item Run 2SLS using the remaining $K$ instruments (just identified) to get $\widehat{\beta}_{2SLS}^{*}$.
	\item Evaluate the \alert{moment condition}: $g_n(\widehat{\beta}_{2SLS}^*) = \frac{1}{n}\sum_i (Y_i - X_i \widehat{\beta}_{2SLS}^{*})\, z_i^*$.
	\item Construct the quadratic form: $J = n\, g_n(\widehat{\beta}_{2SLS}^*)'\, \widehat{S}^{-1} g_n(\widehat{\beta}_{2SLS}^*) \sim \chi^2_{L-K}$, where $\widehat{S}$ estimates the variance of $\sqrt{n}\,g_n$ (accounting for $\widehat{\beta}^*$ being estimated) and $L-K$ is the number of extra moments.
\end{enumerate}
\end{frame}
...
\begin{enumerate}
	...
	\begin{itemize}
		\item With appropriate clustering/standard errors we want to see $F>10$. That was the old standard; see the post-2020 slide below.
...
\begin{itemize}
	\item Staiger--Stock's $F > 10$ keeps the \emph{bias} of 2SLS under $10\%$ of the OLS bias (with several instruments). It says
...
\begin{itemize}
	...
	the AR interval. \texttt{ivDiag} and \texttt{ivmodel} in R do it; \texttt{ivmodels} in Python.
\end{itemize}
\end{frame}
...
\begin{itemize}
	\item Session 6 wrote $Y_i = \beta_0 + \beta_1 X_i + \varepsilon_i$ with \emph{one} $\beta_1$. In business data the
...
\begin{itemize}
	...
	\medskip
	\item Three assumptions (plus a first stage):
	\begin{enumerate}
...
\begin{itemize}
	...
	want two boys have a third child after a boy and a girl but stop after two boys: defiers.
	\medskip
	...
	subpopulation of compliers, provided some subgroup of compliers matches the defiers in size and average effect. The theorem is
...
\begin{itemize}
	...
	\item Testing it: first stage positive in every subgroup (claim type, size, channel; Dobbie, Goldin and Yang 2018);
	Frandsen, Lefgren and Leslie (2023) also test the reduced form. Our table passes for small, medium, and large claims.
...
$\alpha_k \propto g_k \sum_m s_{mk}\tilde x_m$. Weights can be negative. Here one sector carries almost 40\% of the weight; that is the sector whose share-exogeneity claim needs defending, and its own estimate is what
...
tot <- tapply(R, agent, sum); n_j <- tapply(R, agent, length); d$len <- (tot[agent] - R) / (n_j[agent] - 1)
feols(spend ~ 1 | R ~ len, d, cluster = ~agent)
# Shift-share: build B and instrument
d$B <- as.vector(shares %*% g); feols(y ~ 1 | x ~ B, d, vcov = "hetero")
\end{lstlisting}
\begin{itemize}
	\item Shock-level standard errors: \texttt{ssaggregate} (Borusyak--Hull--Jaravel) or the AKM code; Rotemberg
	weights: \texttt{bartik\_weight} (GPSS). These are Stata-first (the R ports are on GitHub); AKM's is in R (\texttt{ShiftShareSE}). The workbook computes
