...
\begin{itemize}
	...
	y_t = y_0 (1+g_1)(1+g_2)\cdots(1+g_t) \quad \Longrightarrow \quad
	\ln y_t = \ln y_0 + \textstyle\sum_{s=1}^{t} \ln(1+g_s)
	\]
...
\begin{itemize}
	...
	\medskip
	\item Fine for small coefficients: $\beta_1 = 0.05$ is 5.1\%. Not fine for big ones:
	\begin{center}
	\begin{tabular}{ccc}
	$\beta_1$ & ``$100\beta_1$\%'' & exact: $100\left(e^{\beta_1}-1\right)$\% \\
	\hline
	0.05 & 5\% & 5.1\% \\
	0.30 & 30\% & 35\% \\
	0.70 & 70\% & \alert{101\%} \\
...
\begin{itemize}
	...
	\medskip
	\item Modeling log-odds as ${\bf x}'\boldsymbol{\beta}$ \alert{is} logistic regression --- coming in
...
\begin{itemize}
	...
	linear-in-parameters --- the $\text{exp}^2$ term you have already met.
\end{itemize}
\end{frame}
...
\bigskip
\begin{itemize}
	\item Open \texttt{inclass\_session2b.Rmd} (in the \texttt{Fall/L3 - Linear Regression} folder of the course repo).
	\item Reproduce the log-points table, the Walmart leverage demo, and the rare-events arithmetic.
	\item Then work the two problems with a neighbor:
	\begin{enumerate}
		\item Recover a price elasticity (and prove units don't matter)
		\item The log(0) trap (Chen--Roth, live)
	\end{enumerate}
\end{itemize}
\end{frame}
