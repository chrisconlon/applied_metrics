...
\begin{itemize}
	...
	\smallskip
	\item Let's revisit estimating this with the Cornwell and Rupert (PSID) data. 
\end{itemize}
\end{frame}
...
\only<2>{
	Note on interpreting effects with $\log(y_i)$: a dummy multiplies $y$ by $\exp(\beta)$, and $\exp(\beta)\approx 1+\beta$ only for small $\beta$:
\begin{itemize}
	\item $\exp \left( -.3892\right) = .6776$
		\item $\exp \left( .05654\right) = 1.058$
\end{itemize}
}
\only<3>{
	\begin{itemize}
	\item In Regression \#1, what does the coefficient $\beta_{ED}$ mean?
...
\begin{frame}{Caveat: These Are Panel Data}
\small
\begin{itemize}
	\item Cornwell--Rupert is \alert{595 workers observed for 7 years}, not 4,165 independent draws. A worker's
	errors are correlated across years (ability, occupation, location), and ED and FEM never change within a
	worker: the worst case of the Moulton factor from Lecture 3a ($\rho_x = 1$).
	\item Heteroskedasticity-robust SEs do not fix this. Clustering by worker roughly doubles them:
\end{itemize}
\begin{center}
\begin{tabular}{lcccc}
\toprule
 & ED & FEM & UNION & EXP \\
\midrule
Coefficient & 0.0565 & $-$0.3892 & 0.0901 & 0.0404 \\
SE: IID & 0.0026 & 0.0252 & 0.0129 & 0.0022 \\
SE: HC1 & 0.0027 & 0.0240 & 0.0125 & 0.0022 \\
SE: clustered by worker & \alert{0.0056} & \alert{0.0482} & \alert{0.0240} & \alert{0.0042} \\
\bottomrule
\end{tabular}
\end{center}
\begin{itemize}
	\item In R: \texttt{cluster = \textasciitilde ID} with \texttt{ID = rep(1:595, each = 7)} (rows are sorted by worker).
	Conclusions survive, but today's SEs are too small; panel data get their own treatment in Lecture 8a.
\end{itemize}
\end{frame}
...
\begin{align*}
\ln wage_i = \beta_0 + \beta_{exp} Experience_i + \beta_{exp^2} Experience^2_i + X_i'\boldsymbol{\beta} + \varepsilon_i
\end{align*}
...
\begin{frame}{Simulating the Asymptotic Distribution}
\begin{itemize}
\item Given the asymptotic distribution of a parameter estimate
\begin{align*}
\boldsymbol{b}\overset{a}{\sim}\mathcal{N}\left(\boldsymbol{\beta},\boldsymbol{\Sigma}/n\right),
\end{align*}
we have an estimated density function $\hat{f}$. Let $\hat{f}$ be
the multivariate normal density with mean $\boldsymbol{b}$ and
variance $\widehat{\boldsymbol{\Sigma}}/n$.
...
but computing the derivative of $g\left(\cdot\right)$ would be difficult.
For example, when $g\left(\cdot\right)$ represents a complex behavioral
...
\begin{frame}{Bootstrap Marginal Effects in R}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{figure}
\flushleft
...
\begin{itemize}
	...
	\item Let's say the recorded education might be one year more or less than the
...
\begin{figure}
\flushleft
	\includegraphics [height=.8\textheight]	{table_noise}
\end{figure}
\end{column}
\end{columns}	
\end{frame}
...
\begin{itemize}
	...
	\item The last term is mean zero given strict exogeneity, $\E\left[\boldsymbol{\varepsilon} \mid {\bf X},{\bf z}\right]={\bf 0}$.

	\item Note that the second term will not be zero if $\gamma \ne 0$ and ${\bf z}$ is correlated
	with the (non-constant) columns of ${\bf X}$.
...
\begin{itemize}
	...
	\begin{align*}
	\E\left[b_{OLS,k} \mid {\bf X},{\bf z}\right]=\beta_{k}+\gamma\left(\frac{\widetilde{{\bf x}}_{k}'{\bf z}}{\widetilde{{\bf x}}_{k}'\widetilde{{\bf x}}_{k}}\right)
\end{align*}
where $\widetilde{{\bf x}}_{k}$ is the residual from regressing $x_k$ on ${\bf X}_{-k}$, all the regressors besides $x_k$.
The ratio is the slope from regressing ${\bf z}$ on $\widetilde{{\bf x}}_{k}$: the partial covariance of $z$ and $x_k$ over the partial variance of $x_k$.
...
\bigskip
\begin{itemize}
	\item Open \texttt{inclass\_session4.Rmd} (in the \texttt{Fall/L5 - Wage Equation Example} folder of the course repo).
