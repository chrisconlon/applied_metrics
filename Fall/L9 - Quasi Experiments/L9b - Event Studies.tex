...
\begin{itemize}
	...
	drift), and noise. Treated firms could differ in levels (the firm effects absorb that); in this first
	version they are on the same trend as the others.
	\medskip
...
\end{center}
\vspace{-.3cm}
{\small Flat and near zero before, ramping after, sitting on the truth. The dot-dashed orange line is the
...
\begin{itemize}
	...
	$k = -1$.
	\medskip
	\item The static two-way FE coefficient is the average post-period $\hat\tau_k$ minus the average pre-period
	$\hat\tau_k$ (with $\hat\tau_{-1}=0$); absent pre-trends, an equally weighted average of the $\tau_k$, $k \ge 0$. With a ramp, it understates
	the long-run effect. With effects that fade, it overstates it. It is never the number you
	would have asked for.
	\medskip
	\item Confidence bands are pointwise. Eleven pre-period coefficients at $95\%$ each is not a
...
\begin{itemize}
	...
	\medskip
	\item Parallel trends is an assumption about \emph{changes} in $Y(0)$, not about levels of $Y$.
...
\begin{itemize}
	...
	\medskip
	\item \alert{Longer pre-periods} help detect trends; they do not remove the bias once you find one.
	\medskip
	...
	segment, matched on pre-period trajectory, or --- in Week 11 --- a synthetic control.
\end{itemize}
\end{frame}
...
renegotiation). The pre-period now steps up at $k=-2$; since $k = -1$ is the reference, every
...
\begin{itemize}
	...
	$[-10, +10]$. Ending the estimation window before the event window keeps the event out of the estimation sample.
	\medskip
...
market-model prediction. The abnormal return is the gap. Even for the largest surprise, a true $5.9\%$
jump shows up as a $2.6\%$ abnormal return after a bad noise draw, smaller than the move on day $+5$.
One firm tells you little.}
\end{frame}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \alert{Standardized} (Patell 1976): scale each firm's CAR by its own
...
\begin{center}
\input{tab_car_erc.tex}
\end{center}
\medskip
{\small Regress $\widehat{CAR}_i(0, 10)$ on $S_i$ with heteroskedasticity-robust standard errors. The
slope is within sampling error of the truth: the CAR analog of interacting event-time dummies with a covariate.}
\end{column}
\begin{column}{0.5\textwidth}
...
\begin{enumerate}
	...
	\item Reproduce the CAR windows. Then move every firm's announcement to the same calendar day and
