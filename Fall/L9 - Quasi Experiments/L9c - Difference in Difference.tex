...
\title[L9a - DiD]{ Econometrics I}
\subtitle{Lecture 9a: Difference-in-Differences}
...
\begin{description}
  ...
  \item [\alert{Difference in Difference:}] Compare $Y_{i,t}(1)-Y_{i,t-1}(0)$ to $Y_{j,t}(0)-Y_{j,t-1}(0)$  for some $j \neq i$.
...
\begin{frame}{Motivation: Selection on Observables}
Regression and matching on observables (matching: Session 11) have some advantages:
\begin{itemize}
...
\begin{itemize}
\item Treated patients are ``matched'' to control patients based only on \alert{observable characteristics}
\begin{itemize}
\item Ignored \alert{selection on unobservables}.
\end{itemize}
\item Rely on \alert{cross sectional} variation to construct a control group.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  $D_i = 1$, and similarly $\bar{Y}_{0}$ is the sample mean conditional on $D_i = 0$.
...
\begin{itemize}
  ...
  that attempts to deal with selection by focusing on how treatment and control
...
\item Idea: Compare \alert{change} in employment (FTE) in $NJ$ to $PA$:  $\Delta_{DD} = \Delta_{NJ}- \Delta_{PA}$
\begin{itemize}
...
\begin{frame}{Differences in Employment: 2 x 2 Table}
\begin{center}
...
\begin{align*}
Y_{it}&=\beta X_{it} +\alpha \cdot [i \in \text{NJ}] +  \gamma \cdot \text{After}_{t}  + \delta \cdot NJ_i \times After_{t} +u_{it}\\
Y_{it}&=\beta X_{it} +\alpha \cdot [\text{wage gap}_{i}] +  \gamma \cdot \text{After}_{t}  + \delta \cdot \text{wage gap}_{i} \times After_{t} +u_{it}
\end{align*}
\item $\alpha$ is the $NJ - PA$ difference in period $1$
\item $\gamma$ is the change from period $1$ to $2$ in $PA$ (control)
\item $\delta$ is the parameter of interest, the \alert{difference in difference}
\item $\text{wage gap}_{i} = \max\{0, 5.05-w_{i1}\}/w_{i1}$ for NJ stores, $0$ for PA.\\
 (By what proportion must the $t=1$ wage rise to reach the new minimum?)
\end{itemize}
\end{frame}

\begin{frame}{Differences in Employment}
\begin{center}
...
\begin{frame}{Inference with Few Treated Clusters}
\small
\begin{itemize}
\item Card and Krueger's SEs treat 410 stores as independent, but the policy varies by \emph{state}: NJ vs.\ PA.
\item Stores in a state share shocks (local demand, weather). Cluster-robust SEs need many clusters, and
many \emph{treated} clusters; with $G = 2$ you cannot cluster by state at all.
\item The wild cluster bootstrap (Cameron, Gelbach and Miller 2008) fixes \emph{few clusters}, not \emph{few treated
clusters}: with one or two treated clusters it still over-rejects (MacKinnon and Webb 2018).
\item With one treated group and \emph{many} control groups:
\begin{itemize}
\item Conley and Taber (2011): control groups' residuals estimate the distribution of the treated group's error.
\item Randomization inference: assign the policy to each control group in turn (the placebo logic of Session 11).
\item Aggregate to group $\times$ period and use $G-1$ degrees of freedom (Donald and Lang 2007).
\end{itemize}
\item With one control group, as in Card and Krueger, none of these can deliver policy-level inference. Count your
treated (and control) clusters before you report a standard error.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  rather than simple differences. They don't need to be: with
...
\begin{frame}{Difference in Difference: Setup}
\begin{align*}
\text{Observed outcome in period } 1&=Y_{i 1}(0)\\
\text{Observed outcome in period } 2&=\left\{ \begin{array}{ll}
...
\begin{frame}{Difference in Difference: Decomposition}
\small
...
\begin{align*}
\E[\Delta Y_{it} \mid D_{i2}=1]  = \overbrace{\E\left[Y_{i2}(1) - Y_{i2}(0) \mid D_{i2}=1 \right]}^{\alert{ATT}} +  \overbrace{\E\left[Y_{i2}(0) - Y_{i1}(0) \mid D_{i2}=1 \right]}^{\alert{\gamma(1)}}
\end{align*}
...
\small
\begin{itemize}
\item If $\gamma(1) = \gamma(0)$ then the trend terms cancel and we are left with $\Delta_{DD} = \text{ATT}$.
...
\begin{itemize}
\item \alert{Parallel trends} assumes that, absent treatment, we add $\gamma_2 - \gamma_1$ to each unit
\item Because this is \alert{additive} it is not invariant to transformations $f(Y_{it})$ (e.g., taking logs)
\end{itemize}
...
\begin{frame}{Checking Pre-Trend: Card and Krueger (2000)}
\begin{figure}
\centering
...
\begin{frame}{Difference in Differences: Regression Form}
Just like in Card and Krueger, we can write as a regression equation:
\begin{align*}
...
\begin{frame}{The ``Ashenfelter Dip'' (Heckman and Smith 1999)}
\begin{figure}
\centering
...
\begin{frame}{Ashenfelter Dip: Algebra}
\small
\begin{itemize}
\item  \#2 above violates the assumption {\small $\E[Y_{i2}(0) - Y_{i1}(0) \mid D] = \E[Y_{i2}(0) - Y_{i1}(0)]$}. 
\item To see why note that those participating in the program are such
that {\small $Y_{i1}(0) < B$}. Assume for simplicity that the shocks {\small $u$} are {\small $iid$}. Hence {\small $u_{i1} < B- \alpha_i - \gamma_1$}. 
This implies: 
{\small $$\E[Y_{i2}(0)- Y_{i1}(0) \mid D=1] = \gamma_2 - \gamma_1 - \E[u_{i1} \mid u_{i1} <  B-\alpha_i - \gamma_1]$$}
For the control group:
{\small $$\E[Y_{i2}(0) - Y_{i1}(0) \mid D=0] = \gamma_2 - \gamma_1 - \E[u_{i1} \mid u_{i1} >  B-\alpha_i - \gamma_1]$$}
So that:
\begin{align*}
...
\end{align*}
\item This is just the matching assumption on a redefined variable, namely the growth in the outcomes. In its simplest form the approach is implemented by running the regression
\begin{align*}
Y_{it} = \alpha_i + \gamma_t + \delta_i D_{it} + \beta_t' X_i + u_{it}
\end{align*}
which allows for differential trends in the non-treatment growth depending on $X_i$. More generally one can implement propensity score matching on the growth of the outcome variable when panel data is available.
\end{itemize}
\end{frame}
...
\item We can compute two D-i-D here: $\Delta_{DDD} = \Delta_{DD}^{\text{men}} - \Delta_{DD}^{\text{women}}$, where each is the treated-vs-untreated-state DiD within that group.
\item Literally the difference of two difference-in-differences estimators.
...
\begin{itemize}
  ...
  \begin{align*}
  \Delta Y_{i}=\beta_{1} \cdot \Delta D_{i}+\boldsymbol{\beta}_2' \Delta {\bf X}_{i} + \Delta \varepsilon_{i}
  \end{align*}
...
\begin{itemize}
  \item A {\bf staggered diff-in-diff} involves different groups being
    ...
    on this in Lecture 9b.) 

  \medskip
  \item See \href{https://www.sciencedirect.com/science/article/pii/S0304407620303948}{Callaway and Sant'Anna (2021)}, 
    ...
    treatment effects and for alternative estimators. (More on this in Lecture 9b.)
