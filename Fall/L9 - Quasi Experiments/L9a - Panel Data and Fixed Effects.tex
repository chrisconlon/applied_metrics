...
\title[L8a - Panel Data]{ Econometrics I}
...
\begin{itemize}
	...
	\begin{align*}
		...
		x_{it} &= \rho\,\alpha_i + \text{(persistent firm component)} + \text{(year-to-year noise)}, \quad \rho = 0.7
	\end{align*}
...
\end{center}
\vspace{-.3cm}
{\small Grey: all firm-years. Colored: eight firms, each drawn with its own intercept and the common within (FE)
slope, $0.12$ (truth $0.10$). The pooled line runs through the \emph{between-firm} cloud.}
\end{frame}
...
\begin{itemize}
	...
	between-minus-within gap $\hat\beta_B-\hat\beta_{FE}$ that drives the pooled bias, estimated directly.
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\medskip
	\item First differences need exogeneity only for adjacent periods: weaker than strict exogeneity, but
	one-period feedback still breaks it. FE and FD differing a lot is itself a diagnostic.
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\begin{center}
	\input{tab_measurement_error.tex}
	\end{center}
...
is already about $0.7$ or more. RE and FE will be close in \emph{value} whenever they are both consistent; the
...
\begin{itemize}
	...
	$\Cov(\bar x_i, \alpha_i) \ne 0$, RE inherits part of the pooled bias (less than $(1-\theta)$ of it).
...
\end{center}
\vspace{-.3cm}
{\small FE does not care. RE's bias grows with the correlation, less than pooled OLS because
...
\begin{center}
\input{tab_hausman.tex}
\end{center}
\medskip
\begin{itemize}
	\item The classical test works here because FE and RE are the \emph{same model} (both with year effects)
	and the errors are homoskedastic by design. It rejects.
	\item Get the specification wrong --- one-way RE against two-way FE --- and $\hat V_{FE} - \hat V_{RE}$
	turns negative in this very sample: the fragility from the previous slide.
	\item The Mundlak version agrees, rejects decisively, and came with clustered
...
\begin{itemize}
	...
	outcomes, unclustered two-way FE regressions reject a true null of \emph{no effect} up to 45\% of the
...
\begin{itemize}
	...
	$\hat\rho$ is biased down by roughly $0.2$, and $\hat\beta$ picks up some of the damage.
...
\begin{itemize}
	...
	three names, one formula. The weight is the store's reliability: the signal share of its mean's variance.
\end{itemize}
\end{frame}
...
\end{center}
\vspace{-.3cm}
{\small Left: four-week stores (red) are pulled most of the way to the grand mean; 64-week stores keep most of
...
\begin{itemize}
	...
	\item Believing $\hat\alpha_i$ causally also needs as-good-as-random \emph{assignment}
	of units to $i$: patients are not randomly assigned to physicians (movers designs: Chetty--Hendren for
	places, Finkelstein--Gentzkow--Williams for physicians).
	\item Nonlinear models (logit with firm FE) add the \alert{incidental parameters} problem:
	$\hat\beta$ is inconsistent for fixed $T$.
\end{itemize}
\end{frame}
