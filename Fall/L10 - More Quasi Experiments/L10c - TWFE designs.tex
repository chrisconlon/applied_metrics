...
\title[L9b - Staggered TWFE]{ Econometrics I}
\subtitle{Lecture 9b: Staggered Adoption and Two-Way Fixed Effects}
...
\begin{itemize}
\item \href{https://www.sciencedirect.com/science/article/pii/S0304407620303948}{Callaway and Sant'Anna (2021)}
\item \href{http://goodman-bacon.com/pdfs/ddtiming.pdf}{Goodman-Bacon (2021)}
\item \href{https://www.aeaweb.org/articles?id=10.1257/aer.20181169}{de Chaisemartin and D'Haultf{\oe}uille (2020)}
\item \href{https://arxiv.org/abs/1804.05785}{Sun and Abraham (2021)}
...
\item Parallel trends $\mathbb{E}[Y_{it}(0) - Y_{i,t-1}(0) \mid D_{it}=1] =\mathbb{E}[Y_{it}(0) - Y_{i,t-1}(0) \mid D_{it}=0]$
\item If parallel trends holds: $\tau = ATT = \mathbb{E}[Y_{it} - Y_{i,t-1} \mid D_{it}=1]-\mathbb{E}[Y_{it} - Y_{i,t-1} \mid D_{it}=0]  $
\end{itemize}
\end{frame}

\begin{frame}{Simple Question?}
When does TWFE deliver the $ATT=\mathbb{E}[\tau_{it} \mid D_{it}=1]$?
\begin{align*}
...
y_{it} = x_{it} \beta + \tau_{it} \cdot  D_{it} + \eta_i + \lambda_t + \varepsilon_{it}
\end{align*}
Three different control groups when we compare $Y_{it} \mid D_{it}=1$:
\begin{enumerate}
...
\begin{frame}{What do we get? (Goodman-Bacon 2021)}
\centering
...
\begin{frame}{Assuming Equal Groups (Goodman-Bacon 2021)}
\centering
\includegraphics[height=0.9\textheight]{resources/gb_fig4.png}
\end{frame}


\begin{frame}{Time varying TE $\tau_{it}$ (Goodman-Bacon 2021)}
\centering
\includegraphics[height=0.9\textheight]{resources/gb_fig3.png}
\end{frame}


\begin{frame}{Application: (Goodman-Bacon 2021)}
\centering
\includegraphics[height=0.9\textheight]{resources/gb_tab1.png}
\end{frame}


\begin{frame}{Event Study Plot (Goodman-Bacon 2021)}
\centering
\includegraphics[height=0.9\textheight]{resources/gb_fig5.png}
\end{frame}

\begin{frame}{Decomposition Plot (Goodman-Bacon 2021)}
\centering
\includegraphics[height=0.9\textheight]{resources/gb_fig6.png}
\end{frame}

\begin{frame}{Application: Estimates (Goodman-Bacon 2021)}
\centering
...
\begin{frame}{Recap}
\begin{itemize}
\item The Goodman-Bacon (2021) paper tells us what we \alert{are} measuring with TWFE.
...
\begin{align*}
\E \left[Y_{t}(0)-Y_{t-1}(0) \mid G=g\right]&=\E\left[Y_{t}(0)-Y_{t-1}(0) \mid C=1\right] \\
\E \left[Y_{t}(0)-Y_{t-1}(0) \mid G=g\right]&=\E \left[Y_{t}(0)-Y_{t-1}(0) \mid D_{s}=0, G \neq g\right]
\end{align*}
...
\begin{align*}
ATT(t,g) = \E \left[Y_{t}(g)-Y_{t}(0) \mid G=g\right]
\end{align*}
