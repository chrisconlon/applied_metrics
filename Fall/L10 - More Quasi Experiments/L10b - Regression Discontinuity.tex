...
\title[L10 - RDD]{ Econometrics I}
\subtitle{Lecture 10: Regression Discontinuity}
...
\begin{align*}
\lim_{x\rightarrow c^{+}} \Pr(D_i | X_i = x) \neq \lim_{x\rightarrow c^{-}}\Pr(D_i | X_i = x)
\end{align*}
...
\item For now we focus on the \alert{sharp discontinuity}:\\
 $\Pr(D_i | X_i \geq c) =1$ and $\Pr(D_i | X_i < c) =0$
...
\item We need that $\E[Y(1)  \mid X]$ and $\E[Y(0) \mid X]$ both be continuous at $X=c$.
\begin{itemize}
...
\begin{eqnarray*}
Y_i = \beta_0 + \tau_i \cdot D_i + X_i \beta + \epsilon_i
\end{eqnarray*}
...
\begin{eqnarray*}
Y_i = f(x_i) + \tau \cdot D_i  + \epsilon_i
\end{eqnarray*}
...
\end{itemize}
\item Local Linear/Polynomial Regression (next semester)
\end{enumerate}
...
\begin{itemize}
\item Plot of $\Pr[D \mid X]$ so that we can see the discontinuity
\item Plot of $\E[Y \mid X]$ so that we see discontinuity there also
\item Plot of $\E[W \mid X ]$ so that we don't see a discontinuity in controls.
...
\begin{align*}
\lim_{x\rightarrow c^{+}} \Pr(D_i \mid X_i = x) \neq \lim_{x\rightarrow c^{-}} \Pr(D_i \mid X_i = x)
\end{align*}
\begin{itemize}
\item We need a discontinuous jump in probability of treatment, but it doesn't need to be $0 \rightarrow 1$.
\begin{align*}
\tau_i(c) = \frac{\lim_{x\rightarrow c^{+}} \E(Y_i \mid X_i = x) - \lim_{x\rightarrow c^{-}}\E(Y_i \mid X_i = x)}
{\lim_{x\rightarrow c^{+}} \Pr(D_i \mid X_i = x) - \lim_{x\rightarrow c^{-}}\Pr(d_i \mid X_i = x)}
...
\end{itemize}
\item Compute the average value:  $b_x^j=\E[x_i \mid x_i \in \text{bin}_j]$
\item Compute the average value:  $b_y^j=\E[y_i \mid x_i \in \text{bin}_j]$
...
\begin{align*}
y_{i}=\mu\left(x_{i}\right)+\mathbf{w}_{i}^{\prime} \gamma+\epsilon_{i}, \quad \E\left[\epsilon_{i} \mid x_{i}, \mathbf{w}_{i}\right]=0
\end{align*}
\begin{itemize}
