...
\begin{column}{0.5\textwidth}
\begin{itemize}
	\item If we plotted total food expenditure (rather than the expenditure share), the heteroskedasticity would go in the other direction.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{Heteroskedasticity vs. Correlation}
\begin{itemize}
	\item Recall that we defined the homoskedasticity assumption as:\[
	...
	\item The terminology can be misleading here, because what people refer to as ``heteroskedasticity-robust''
...
\begin{itemize}
	...
	\item With clustering, the middle term also picks up within-group cross products:\[
	\widehat{\Var}_{clu} = n^{-1} \sum_{i=1}^{n}\sum_{j=1}^{n} {\bf x}_i\,  {\bf x}'_j \, e_i\, e_j \cdot \mathbb{I}\left[g\left(i\right)=g\left(j\right)\right]
	\]
	where the indicator is 1 when $i$ and $j$ are in the same group and 0 otherwise.
	The $i=j$ terms are exactly the heteroskedasticity-robust sum above.
...
\begin{itemize}
	...
	\item Note that this estimator adds extra terms ${\bf x}_i {\bf x}_j' e_i e_j$ (covariance terms) to the estimate of variance,
	so it makes standard errors larger when ${\bf x}_i \varepsilon_i$ is positively correlated within groups
	--- typically because \emph{both} the regressor and the error are.

	\item Rough guide (Moulton): the variance is inflated by about $1 + \left(n_g - 1\right)\rho_x \rho_\varepsilon$,
	where $\rho_x$ and $\rho_\varepsilon$ are the within-group correlations of the regressor and the error.
	A regressor that varies only at the group level ($\rho_x = 1$) is the worst case; one that is independent
	within groups ($\rho_x \approx 0$) barely changes.

	\item Thus, if standard formulas are used in the presence of cluster-correlated disturbances
	and regressors, standard errors will be too small.
...
\begin{itemize}
	...
	\qquad \text{(GLS: ${\bf W} = \Var\left(\boldsymbol{\varepsilon}|{\bf X}\right)^{-1}$, not necessarily diagonal)}
	\]
