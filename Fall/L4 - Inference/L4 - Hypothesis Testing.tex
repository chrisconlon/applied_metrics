...
\begin{frame}{$t$-Statistics for OLS Parameters (homoskedasticity)}
...
\begin{itemize}
	...
	\item Even though we would naively reject, in actuality \emph{not significant}
...
\begin{itemize}
	...
	\item The dashed lines are the rejection regions for each of the individual t-tests.  (5\% level)
	\item Now, notice that $t_1 = -2$, $t_2=-2$ would be a rejection according
	to each of the individual tests, but it is not a rejection of the joint test.
\end{itemize}
\end{column}
\end{columns}
...
\begin{itemize}
	...
	and you should \alert{expect} one segment ($20 \times 0.05 = 1$) to ``respond significantly,''
	effect or no effect.
...
\begin{itemize}
	...
	the true parameter (\alert{coverage}) is $1-\alpha$: exactly with normal errors, approximately
	(as $n$ grows) otherwise.
	That is, if $\alpha=.05$, this is called a 95\% confidence interval, and there
	is a 95\% chance it will contain the true parameter.
...
\begin{frame}{Bootstrap: Using the Draws}
\small
...
Three common ways to construct bootstrap CI's (a fourth, better one on the next slide):
\begin{enumerate}
\item Percentile: sort  $\hat{\theta}^{*}$ then take $CI: [\hat{\theta}^{*}_{\alpha/2},\hat{\theta}^{*}_{1-\alpha/2}]$.
\item Asymptotic Normal:  $CI: \hat{\theta} \pm 1.96 \sqrt{V(\hat{\theta}^{*})}$. (CLT).
\item Basic: let $W= \hat{\theta} -\theta$, with quantiles $w_{q}$. If we knew the distribution of $W$ then $\Pr(w_{\alpha/2} \leq W \leq w_{1-\alpha/2}) = 1-\alpha$, so:
\begin{eqnarray*}
...
\end{eqnarray*}
Unlike the percentile interval, this shifts \emph{against} the estimated bias.
\end{enumerate}

\end{frame}


\begin{frame}{Bootstrap: Confidence Intervals (Bootstrap-$t$)}
\small
\small
\begin{enumerate}
\setcounter{enumi}{3}
\item Bootstrap-$t$ (percentile-$t$): bootstrap the \alert{$t$-statistic} instead of $\hat{\theta}$ itself.
\begin{itemize}
\item In each bootstrap sample $b$, compute $\hat{\theta}^{*}_{(b)}$ \emph{and} its standard error $se^{*}_{(b)}$ (e.g., robust SE), and form
\begin{eqnarray*}
T^{*}_{(b)} = \frac{\hat{\theta}^{*}_{(b)} - \hat{\theta}}{se^{*}_{(b)}}
\end{eqnarray*}
\item With quantiles $T^{*}_{q}$ and $se(\hat{\theta})$ from the original sample:
\begin{eqnarray*}
CI: [\hat{\theta} - se(\hat{\theta})\, T^{*}_{1-\alpha/2},\ \hat{\theta} - se(\hat{\theta})\, T^{*}_{\alpha/2}]
\end{eqnarray*}
\end{itemize}
\end{enumerate}
\begin{itemize}
\item Same logic as the basic interval, but $T = (\hat{\theta}-\theta)/se(\hat{\theta})$ is \alert{asymptotically pivotal}
(its limit distribution is free of unknown parameters); $W = \hat{\theta}-\theta$ is not.
\item That buys the higher-order accuracy on the next slide. Percentile and basic intervals are only first-order accurate.
\item Cost: you need a standard error inside every bootstrap draw.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Econometricians like the bootstrap because, applied to asymptotically pivotal statistics (the bootstrap-$t$), it is \alert{higher-order accurate} for confidence interval construction (but not the standard errors).
\begin{itemize}
...
\begin{itemize}
	...
	correlation or heteroskedasticity), we need to use somewhat more complex formulas
